...
const ImageGenerator: React.FC = () => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
            ...
            <div className="flex flex-col items-center justify-center h-full bg-gray-900/50 rounded-lg p-4 min-h-[400px] md:min-h-full">
                {isLoading && (
                    <div className="text-center">
                        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-red-500 mx-auto"></div>
                        <p className="mt-4 text-gray-300">Criando sua imagem...</p>
                    </div>
                )}
                {error && <p className="text-red-400 text-center">{error}</p>}
                {generatedImage && (
                    <div className="w-full">
                        <img src={generatedImage} alt="Generated" className="rounded-lg w-full object-contain mb-4" />
                        <button 
                ...
                {!isLoading && !error && !generatedImage && (
                    <div className="text-center text-gray-500">
                       ...
                       <p>Sua imagem gerada a partir do texto aparecerá aqui.</p>
                    </div>
                )}
            </div>
        </div>
